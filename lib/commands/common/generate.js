...
module.exports = function generate(v) {
    ...
    utils.prompt(questions)
        ...
        .then(function(answers) {
            widgetPath = utils.projectPath(answers.name);
            if(answers.standalone === false) {
                display.info('Deleting standalone files');
                var toDelete = [
                    path.resolve(widgetPath, '.*'),
                    path.resolve(widgetPath, '*.json'),
                    path.resolve(widgetPath, 'model.xml'),
                    //path.resolve(widgetPath, 'README.md'),
                    path.resolve(widgetPath, 'CHANGELOG.md'),
                    path.resolve(widgetPath, 'LICENSE'),
                    path.resolve(widgetPath, 'index.dev.html')
                ]
