...
module.exports = function generate(v) {
    ...
    utils.prompt(questions)
        ...
        .then(function(widgetModel) {
            ...
            if(widgetModel.standalone === false) {
                ...
                var toDelete = [
                    ...
                    path.resolve(widgetModel.path, 'icon.png'),
                    path.resolve(widgetModel.path, 'model.xml'),
                    path.resolve(widgetModel.path, 'Makefile'),
        ...
        .then(function() {
            var fulltargetPath = path.resolve(target);
            if(fulltargetPath !== utils.projectPath()) {
                display.info('Generate destination ' + colors.green(path.join(target, widgetModel.name )));
                shell.mkdir('-p', fulltargetPath);
                shell.mv(widgetModel.path, fulltargetPath);
            }
            return widgetModel;
        })
