...
function writeFiles(params) {
    utils.map(params.files, function(file) {
        ...
        switch (fileType) {
            ...
            break;
            default: fs.writeFileAsync(file.filePath, file.content, 'utf8');
        };
    });
...
function checkVersion(version, input) {
    ...
    return new Promise(function(resolve, reject) {
        ...
        var output = utils.assign({
            ...
            nextVerison: semver.inc(version, inc, suffix)
        }, input);
        resolve(output);
        return output;
    });
...
function confirmNextVersion(params) {
    ...
    return utils.prompt(question).then(function(answer) {
        return utils.assign(answer, params);
    });
}
/**
...
function updateXMLFiles(params) {
    ...
    return promiseModelXML.then(utils.xml2json)
        .then(function(json) {
            ...
            var properties = json.catalog.feature.properties.property;
            try {
                var version = utils.filter(properties, '$.name', 'version')[0].value._;
                contentXML = xml.replace(version, params.nextVerison);
            } catch (err) {
            ...
            var warn = utils.template('Unable to find file ${name} ... skiping ')({
                name: colors.info(filename)
            });
            display.warn(warn);
            return params;
        });
 ...
 * @return {object}        Chained params + .json's
 */
function updateJSONFiles(params) {
    ...
    return Promise.all([
        ...
        var bowerJson = arr[1];
        packageJson.version = params.nextVerison;
        bowerJson.version = params.nextVerison;
        params.files.push({
...
function updateREADME(params) {
    ...
    docs.register('version', params.nextVerison);
        ...
        .then(function(content) {
            ...
            content = utils(messages)
                .filter(function(message) {
                    return ! message.includes(excludesMessages);
                })
                .map(function(message) {
                    return messageTpl({message: message})
                })
                .unshift(headerTpl({
                    version: params.nextVerison,
...
function success(params) {
    display.info('Bumped to version:', colors.info(params.nextVerison));
}
/**
 * Main entry
 * @param  {object} input cli input args
