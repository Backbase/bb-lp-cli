...
function compile(entry, options) {
    return new Promise(function(resolve, reject) {
        ...
        gulp.src(entry, { base: options.target })
            ...
            .pipe(plugins.header(utils.banner(pkg, config)))
...
function copyAssets(entry, options) {
    var fontGlob = path.join(options.target, '**', '*.{ttf,woff,woff2,eot,svg}');
    ...
    gulp.task('copyBowerAssets', ['copyThemeAssets'], function() {
        var assets = gulp.src([
            path.join(options.target, 'bower_components', fontGlob),
            path.join(options.target, 'bower_components', imageGlob),
            path.join(options.target, '!bower_components', 'theme', '**')
        ], { follow: true });
        return assets
            .pipe(plugins.debug({ title: 'copying' }))
