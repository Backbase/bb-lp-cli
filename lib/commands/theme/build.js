...
function getConfig(cmdArguments) {
    ...
    var config = utils.defaults({
        ...
        ie: !cmdArguments.options['disable-ie'],
...
function run(config) {
    if (config.watch) {
        gulp.watch(config.watchFiles, []).on('change', function(event) {
            utils.display('File ' + event.path + ' was ' + event.type + ', running build...');
            run(config);
        });
    }
...
function buildTheme(bowerJson, options) {
    ...
    return utils.reduce(['ie', 'compress', 'assets', 'import'], function(promise, action) {
