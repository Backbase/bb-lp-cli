...
exports.paths = utils.extend( {
    ...
    styles: './styles',
    test: './test',
    index: './widget-standalone.html',
    routes : {
        '/support': path.join(__dirname, '../node_modules')
    }
}, config.paths || {} );
...
exports.data = utils.extend({
    route :'',
    files: ['./**/services/**/*.raml', '!./**/node_modules/**/+(examples|example|test)/**']
}, config.data);
...
exports.proxies = utils.extend( {
    ...
    '//services/rest': 'http://localhost:3030/', // $(servicesPath) is replaced by '' in standalone mode
    '/undefined/': 'http://localhost:3000/' // lp-template adds $(itemRoot) variable to template, which is replaced to undefined
}, config.proxies || {} );
