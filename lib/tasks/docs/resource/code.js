...
module.exports = _.mixin({
	...
	dependencies: function(){
		return shell
			.find('.')
			.filter(function(file){ return !/node_modules|bower_components|dist|test|\.git/.test(file) && file.match(/\.js$/) })
	...
	update: function(files){
		...
		var report = _.extend(escomplex.analyse(_.map(cleanDependencies, function(name) {
			return {
				path: name,
				code: files[name]
			};
