...
module.exports = _.mixin({
	...
	update: function (files) {
		...
		var cssRules = _(cleanDependencies)
			.filter(RegExp.prototype.test.bind(/dist.*styles.*css$/))
			.reduce((memo, fileName) => {
				var parsed = css.parse(files[fileName], {
					silent: true
				})

				memo = memo.concat(_.property('stylesheet.rules')(parsed));
				return memo;
			}, []);
