...
gulp.task('build:scripts', [], function() {
    ...
    var entry = path.resolve(config.main);
    var target = config.customTarget ? utils.bowerMainJs(config.customTarget) : config.mainOutPath || path.normalize(mainJS.replace('./src', './'));
...
gulp.task('start', ['test:lint', 'build:scripts', 'build:styles', 'server'], function() {
    ...
    // dist files to watch (everything, exluding dependencies, scripts & styles).
    var distFiles = [
        '**',
        '!**/node_modules/**/*.js',
        '!**/bower_components/**/*.js',
    ...
    gulp.watch(scriptSrc, watchJsTasks);

    // watch styles
    gulp.watch(styleSrc, ['build:styles']);

    // watch dist
    var watchDistTasks = ['server:reload'];
    if (config.import) {
        watchDistTasks.push('import:item');
    }
    gulp.watch(distFiles, watchDistTasks);
});

gulp.task('import:item', [], utils.debounce(function() {
    portal.getRestClient(config).then(function(bbRestClient) {
        ...
        return bbImport.import.importItem(bbRestClient, item).then(function(result) {
            ...
            var message = utils.template('Imported ${name}${version} : ${status} ${info}');
    ...
    .catch(function(err) {
        ...
        // process.exit(1);
        display.warn('Error importing:', colors.warn(err.context));
    });
}, 500));

gulp.task('import:all', [], utils.debounce(function() {
    portal.getRestClient(config).then(function(bbRestClient) {
    ...
    .catch(function(err) {
        ...
        display.warn('Error importing:', colors.warn(err));
    });
}, 500));
