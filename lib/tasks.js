        ...
        plugins: [
            ...
            new webpack.SourceMapDevToolPlugin({
                ...
                moduleFilenameTemplate: function(info) {
                  var tpl = 'webpack:///';
                  if(info.resource.startsWith('./')){
                    return tpl + config.name + '/' + info.resource;
                  }
                },
...
gulp.task('server', [], function() {
    ...
    var defaultOption = {
        ...
        reloadDebounce: 1000,
        reloadDelay: 300,
        startPath: path.dirname(paths.index),
        server: {
            baseDir: baseDir,
            minify: true,
            cors: true,
...
gulp.task('test:lint', function() {
    ...
    if (utils.isUndefined(config.entry) || !path.extname(utils.bowerMainJs(config.entry.path))) {
        return true;
    }
    ...
    src = src
        .concat(utils.bowerMainJs(config.entry))
        .filter(onlyJS);
...
gulp.task('start', ['test:lint', 'build:scripts', 'build:styles'], function() {
    ...
    var watchJSlist = utils.union(scriptSrc, utils.bowerMainJs(config.entry), [
        '!./**/node_modules{,/**/*}', // ignore **/node_modules
        '!./**/bower_components{,/**/*}', // ignore **/bower_components
        '!./**/dist{,/**/*}', // ignore **/dist
        '!dist{,/**/*}', // ignore **/dist
    ...
    gulp.watch(watchJSlist, watchJsTasks);
    ...
    gulp.watch([
        config.paths.index,
        config.paths.components + '/index.dev.html',
        'model.xml',
        'index.html'
    ], ['server:reload']);
...
gulp.task('deploy:item', [], utils.debounce(function() {
    ...
    return bbImport.import.importItem(config.bbrest, item).then(function(result) {

        var message = utils.template('Deploying item to portal ${name}${version}');
...
gulp.task('build:styles', ['build:images', 'build:css', 'compile:scss'], function() {
    ...
    return gulp.src(utils.resolve(config.paths.styles, 'base.less'))
        ...
        //.pipe(g.rename(config.name + '.css')) //do not rename old less files
        .pipe(g.postcss([autoprefixer(config.autoprefixer)]))
        .pipe(server.reload({
            stream: true
        }))
...
exports.build = function(args) {
    ...
    return new Promise(function(resolve, reject) {
        runSeq(['build'], utils.callbackPromiseResolver(resolve, reject));
    });
};
...
exports.build.bundleTemplates = function() {
    return new Promise(function(resolve, reject) {
        runSeq(['build:bundleTemplates'], utils.callbackPromiseResolver(resolve, reject));
    });
};
...
exports.start = function() {
    ...
    if (config.deploy) {
        portal.getConfig().then(function(bbRestConfig) {
          // too late now,
          // this config becomes the state of the app. :)
          config.bbrest = new BBRest(bbRestConfig);
