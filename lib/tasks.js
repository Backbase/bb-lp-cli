...
gulp.task('start', ['test:lint', 'build:scripts', 'build:styles'], function() {
    ...
    if (config.deploy) {
        watchDistTasks.push('deploy:item');
    }
...
gulp.task('deploy:all', [], utils.debounce(function() {
    portal.getRestClient(config).then(function(bbRestClient) {
        return bbImport.import.importCollection(bbRestClient, utils.projectPath()).then(function(results) {
            var message = utils.template('Deploing item to portal ${name}${version} : ${status} ${info}');
...
gulp.task('compile:scss', [], function() {
    ...
    var size = gulpSize();
    var entry = utils.getStyleEntry(config.entry.path);
    if(utils.isUndefined(entry)) {
        entry = utils.resolve(config.paths.styles, 'index.scss');
    }
...
gulp.task('build:styles', ['build:images','build:css', 'compile:scss'], function() {
    ...
    var entry = utils.getStyleEntry(config.entry.path);
    if(utils.isUndefined(entry)) {
        entry = utils.resolve(config.paths.styles, 'base.scss');
    }
    return gulp.src(entry)
        // Use less compiler
        .pipe(g.sourcemaps.init())
...
gulp.task('build:css', function() {
    ...
    var entry = utils.getStyleEntry(config.entry.path);
    if(utils.isUndefined(entry)) {
        entry = utils.resolve(config.paths.styles, '*.css');
    }
