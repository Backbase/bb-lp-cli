...
var prependBanner = function() {
    var BANNER = '/* Build with ${ pkg.name } @ v${ pkg.version } */\n';
...
gulp.task('compile:scripts', [], function() {
    ...
    if (config.withModuleId) {
        ...
        webpackConf.output.library = utils.camelCase(config.name);
    }
...
gulp.task('start', ['test:lint', 'build:scripts', 'build:styles', 'server'], function() {
    // watch scripts
    gulp.watch([
        config.paths.styles + '/**/*.(less|scss)',
        config.paths.scripts + '/**/*.(less|scss)',
        config.paths.scripts + '/**/*.js'
...
gulp.task('build:scripts', ['compile:scripts'], function() {
    ...
    var fullpath = utils.resolve(config.paths.target, config.mainOutPath);
    return gulp.src([
            fullpath
        ])
        .pipe(g.uglify({
        ...
        .pipe(prependBanner())
        .pipe(g.size({
            title: 'Scripts file size: ',
            showFiles: true
        }))
        .pipe(gulp.dest(path.join(config.paths.target, 'scripts')));
});
...
// Build styles
gulp.task('build:styles', ['compile:scss'], function() {
