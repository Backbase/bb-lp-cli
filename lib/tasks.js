...
gulp.task('compile:scripts', [], function() {
    var entry = [ ];
    // Check for base.less.
    if (fs.existsSync(config.paths.styles + '/base.less')) {
        entry.push(config.paths.styles + '/base.less');
    }
    entry.push(config.main);
...
gulp.task('start', ['test:lint', 'build:scripts', 'compile:styles', 'server'], function() {
    // watch scripts
    gulp.watch([
        config.paths.scripts + '/**/*.js'
