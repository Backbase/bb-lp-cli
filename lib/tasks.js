...
gulp.task('start', ['test:lint', 'build:scripts', 'build:styles', 'server'], function() {
    ...
    var distFiles = [
        ...
        '!**/node_modules/**/*.js',
        '!**/bower_components/**/*.js',
    ].concat(scriptSrc.map(function(src) { return '!' + src.replace(/^\.\//,''); }))
    .concat(styleSrc.map(function(src) { return '!' + src.replace(/^\.\//,''); }));
...
gulp.task('import:item', [], utils.debounce(function() {
    portal.getRestClient(config).then(function(bbRestClient) {
        bbImport.import.importItem(bbRestClient, {
            name: config.name,
            version: config.version,
            srcDir: '.'
        }).then(function(result) {
            display.info('Imported ', colors.info(config.name) + ': ' +
                result.statusCode + ' ' + result.statusInfo);
        });
    })
    .catch(function(err) {
        display.warn('Error importing: ', colors.error(err));
    });
}, 500));
