...
gulp.task('compile:scripts', [], function() {
    ...
    if (excludes && excludes.length) {
        var mainPath = createNewEntry(excludes, main, target);    
    }

    var webpackConf = utils.deepMerge(globalWebpackConf, {
        entry: mainPath && mainPath.length ? mainPath : main,
        output: {
            filename: target
        },
        plugins: []
...
exports.build = function(args) {
    ...
    config.excludes = customConfig.externals
        ? customConfig.externals
        : args.excludes ? args.excludes : null;

    config.customTarget = customConfig.target
        ? customConfig.target
        : args.target ? args.target : config.main;
